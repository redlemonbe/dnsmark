...
Status as of **v2.2.1**. User-facing methodology lives in
...
- One worker per **bound** RX queue, capped to the NIC-local physical-core budget
  ...
  NIC-local **physical** cores (no HT sibling, no real-time scheduling). Per-worker
  local in-flight table; **fixed source port per worker**. The generator steers its
  own RSS indirection table to span exactly the bound queues
  (`ethtool -X equal <queue_count>`) so every response lands on a bound
