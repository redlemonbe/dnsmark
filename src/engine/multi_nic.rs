...
fn merge_snapshots(snaps: &[StatsSnapshot]) -> StatsSnapshot {
    ...
    StatsSnapshot {
        ...
        // Link speed sums across NICs; avg reply size is weighted by each NIC's reply rate
        // (Σ bytes/s ÷ Σ replies/s), so the aggregate % of line rate is exact.
        link_mbps: {
            let m: u64 = snaps.iter().filter_map(|s| s.link_mbps).sum();
            if m > 0 { Some(m) } else { None }
        },
        server_rx_avg_bytes: {
            let num: f64 = snaps.iter()
                .filter_map(|s| Some(s.server_rx_avg_bytes? * s.server_rx_qps?))
                .sum();
            let den: f64 = snaps.iter()
                .filter(|s| s.server_rx_avg_bytes.is_some())
                .filter_map(|s| s.server_rx_qps)
                .sum();
            if den > 0.0 { Some(num / den) } else { None }
        },
        // Aggregate ramp capacity = sum of the per-link NIC-verified knees (each link
        // saturates its own NIC independently), not an average over the ramp-up.
        ramp_capacity: {
            let c: f64 = snaps.iter().filter_map(|s| s.ramp_capacity).sum();
...
pub async fn run_multi_nic(config: Arc<Config>) -> anyhow::Result<StatsSnapshot> {
    ...
    // PER-NIC shutdown: in --ramp each link must ramp to ITS OWN saturation and report
    // independently. A single shared shutdown let the first NIC to saturate kill the others
    // before they finished (only one link's summary printed, no aggregate). Each NIC now
    // owns its shutdown; Ctrl-C still stops every run (each run's own select catches it).
    ...
    // Print per-NIC breakdown. In --ramp the meaningful per-link figure is the DSD knee
    // (NIC-verified Capacity), NOT avg_qps — which averages the whole ramp-up and reads
    // far below the knee; show the Capacity there instead.
    if config.nic_stats || !config.quiet {
        println!("\n── Per-NIC breakdown ──────────────────────────────────");
        for (addr, snap) in &per_nic {
            let iface = crate::autodetect::iface_for_addr(*addr)
                .unwrap_or_else(|| "?".to_string());
            match snap.ramp_capacity {
                Some(cap) => println!(
                    "  {addr} ({iface})  capacity={:.0} qps (NIC-verified knee)  p99={} µs",
                    cap, snap.p99_us,
                ),
                None => println!(
                    "  {addr} ({iface})  sent={} done={} qps={:.0}  p99={} µs",
                    snap.queries_sent, snap.queries_completed, snap.avg_qps, snap.p99_us,
                ),
            }
        }
        println!("────────────────────────────────────────────────────────");
