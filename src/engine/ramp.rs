...
impl RampController {
    ...
    ///
    /// A step is "sustained" when the generator reached ~the target (offer_ok) AND
    /// the median latency stayed under the SLO. Phase 1 doubles from 100k to bracket
    /// the max between the last sustained target and the first failed one; phase 2
    /// bisects inside that bracket (e.g. 4M ok / 8M fails -> 6M, 7M ...) until within
    /// tolerance — no coarse fallback to the last power of two.
