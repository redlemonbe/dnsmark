...
impl InFlight {
    ...
    /// Expire slots older than `timeout`; returns the count of expired (timed-out) slots.
    /// A timed-out query is a loss, not a completion: callers count it via `inc_timeout()`
    /// and never record it into the latency histogram (the histogram holds real response
    /// latencies only). No caller needs the per-slot age, so none is computed or returned.
    ...
    /// Drain ALL non-zero slots (end-of-run); returns the count. Queries still in flight
    /// at end-of-run are losses (no response arrived), counted via `inc_timeout()`.
    pub fn drain_all(&self) -> usize {
        let mut n = 0usize;
        for s in self.slots.iter() {
            if s.swap(0, Ordering::Relaxed) != 0 {
                n += 1;
            }
        }
        n
...
pub struct UnifiedCfg {
    ...
    pub ramp:            bool,  // ramp mode → sample RTTs in firehose for the p50 SLO gate
}
// Per-interface unified configs. Multi-NIC runs one engine::run per NIC, each with its
// OWN qps_per_worker (its own ramp controller); a single global config made both NICs'
// workers read the FIRST NIC's qps, so the two ramp controllers fought (dual-NIC --ramp
// declared saturation at ~1.25M). Keyed by iface so each stack uses its own pacing.
static UNIFIED_CFGS: OnceLock<Mutex<std::collections::HashMap<String, Arc<UnifiedCfg>>>> =
    OnceLock::new();

/// Called by the engine (per NIC) before `start_xdp_receive_path` to enable the unified
/// (RX+TX, one thread per queue/core) datapath for `iface`. Without it, the legacy split
/// sender/receiver path is used.
pub fn set_unified_cfg(iface: String, cfg: UnifiedCfg) {
    UNIFIED_CFGS.get_or_init(|| Mutex::new(std::collections::HashMap::new()))
        .lock().insert(iface, Arc::new(cfg));
}

/// Look up the unified config for `iface`. Falls back to the sole entry when there is
/// exactly one (single-NIC, or a virtual→parent iface rename) so existing behaviour holds.
fn unified_cfg_for(iface: &str) -> Option<Arc<UnifiedCfg>> {
    let m = UNIFIED_CFGS.get()?.lock();
    if let Some(c) = m.get(iface) { return Some(c.clone()); }
    if m.len() == 1 { return m.values().next().cloned(); }
    None
}
...
fn xdp_unified_worker(
    ...
    // 1/64 of ids are latency-sampled in ramp/firehose (enough for percentiles, cheap).
    const LAT_SAMPLE_MASK: u16 = 0x3F;
    ...
    loop {
        ...
        if headroom > 0 {
            ...
            if !descs.is_empty() {
                ...
                if enq > 0 {
                    ...
                    // Closed-loop: register ALL ids for matching/RTT + backpressure.
                    // Ramp (firehose + track_latency): register a 1/64 SAMPLE so the ramp's
                    // p50 SLO gate has RTTs without the cross-core cost of matching every
                    // packet (throughput is still counted per-queue on RX). Pure flood
                    // (firehose, no ramp): register nothing — responses are counted, not matched.
                    if !firehose {
                        ...
                        for &id in ids_to_register.iter().take(enq) {
                            if id & LAT_SAMPLE_MASK == 0 { in_flight.insert(id); }
                        }
                    }
            ...
            for desc in &rxds {
                ...
                if let Some((id, rcode)) = parse_dns_from_frame(frame) {
                    if firehose {
                        ...
                        rc_acc += 1;
                        // Ramp: record the 1/64-sampled RTTs so the p50 SLO gate has data.
                        if track_latency && id & LAT_SAMPLE_MASK == 0 {
                            if let Some(rtt_us) = in_flight.take(id) { stats.record_latency_us(rtt_us); }
                        }
        ...
        if worker_id == 0 && now.duration_since(last_timeout) >= Duration::from_millis(10) {
            let expired = in_flight.sweep_expired(timeout_dur);
            if expired > 0 {
                for _ in 0..expired {
                    stats.inc_timeout();
                }
                if max_outstanding > 0 {
                    global_in_flight.fetch_update(Ordering::Relaxed, Ordering::Relaxed,
                        |x| Some(x.saturating_sub(expired))).ok();
                }
            }
            last_timeout = now;
        }
...
fn do_start_xdp_receive_path(
    ...
    let unified_cfg = unified_cfg_for(iface);
    let firehose = matches!(unified_cfg, Some(ref cfg) if cfg.max_outstanding == 0);
    ...
    for q in 0..queue_count {
        ...
        if let (Some(cfg), Some(hdr)) = (unified_cfg.clone(), frame_hdr_opt.clone()) {
            let sa = SockaddrXdp {
            ...
            std::thread::Builder::new()
                ...
                .spawn(move || {
                    ...
                    let firehose = mo == 0;
                    let track_latency = cfg.ramp;
                    xdp_unified_worker(sock, hdr, sa, wp, st, sd, qps, q as usize, qc, mo, timeout_dur, ifl, gif, firehose, track_latency);
                })
    ...
    if n_unified > 0 {
        if let Some(cfg) = unified_cfg.as_ref() {
