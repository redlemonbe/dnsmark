...
fn xdp_receiver_thread(
    ...
    loop {
        ...
        // Drain completion ring (TX completions — we don't TX, but drain anyway).
        sock.umem.comp.dequeue_all();
...
fn do_start_xdp_receive_path(
    ...
    // Attach one XSK per RX queue (use only queue 0 on VMs with a single queue).
...
pub async fn run_xdp_sender_worker(
    ...
    let _ = timeout_ms; // timeout is handled by the shared XDP receiver thread
    ...
    let sender = std::thread::spawn(move || {
        ...
        let _sock = socket; // keep fd alive
